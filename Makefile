...
#ROOT := /usr
ROOT := /pc2014-data2/pguzowski/snsw_3.0/CadfaelBrew/Cellar/root6/6.08.06
#${ROOTSYS}
...
LINKER       := gcc
DEPENDFLAGS  := -g -O3 -ffast-math -fno-math-errno -Wall ${SEARCHDIRS} -fPIC -Df2cFortran -s -std=c++11
...
EXAMPLELIBS := -lCore -lRIO -lNet -lHist -lGraf -lGraf3d -lGpad -lTree -lRint -lPostscript -lMatrix -lPhysics -lMathCore -lThread -pthread -lm -ldl -rdynamic -lCollieIO -lCollieLimit -lCollieCLHEP
...
${COLLIELIB}/libCollieIO.so: ${IOOBJS} ${IO_SRC}/collieio_dict.o
	gcc -shared -ffast-math -Wl,-soname,libCollieIO.so -o $@ $^ ${LOADLIBS} ${LDFLAGS}

${COLLIELIB}/libCollieLimit.so: ${LIMITOBJS} ${LIM_SRC}/collielimit_dict.o ${LIM_SRC}/wrap.o
	gcc -shared -ffast-math -Wl,--wrap,__ctype_b,-soname,libCollieLimit.so -o $@ $^ ${LOADLIBS} ${LIMLIBS} ${LDFLAGS}
...
examples/IO.exe: examples/IO.o
	gcc -o $@ $^ ${CXXFLAGS} ${LDFLAGS} ${LOADLIBS} ${EXAMPLELIBS} ${SEARCHDIRS} 

examples/IO_contour.exe: examples/IO_contour.o
	gcc -o $@ $^ ${CXXFLAGS} ${LDFLAGS} ${LOADLIBS} ${EXAMPLELIBS} ${SEARCHDIRS} 

examples/Sigmas.exe: examples/Sigmas.o
	gcc -o $@ $^ ${CXXFLAGS} ${LDFLAGS} ${LOADLIBS} ${EXAMPLELIBS} ${SEARCHDIRS} 

examples/CLsb.exe: examples/CLsb.o
	gcc -o $@ $^ ${CXXFLAGS} ${LDFLAGS} ${LOADLIBS} ${EXAMPLELIBS} ${SEARCHDIRS} 

examples/CLs.exe: examples/CLs.o
	gcc -o $@ $^ ${CXXFLAGS} ${LDFLAGS} ${LOADLIBS} ${EXAMPLELIBS} ${SEARCHDIRS} 

Toy/1_CreateCollieIO.exe: hsnToy/1_CreateCollieIO.o
	gcc -o $@ $^ ${CXXFLAGS} ${LDFLAGS} ${LOADLIBS} ${EXAMPLELIBS} ${SEARCHDIRS}

Toy/2_CalculateLimits.exe: hsnToy/2_CalculateLimits.o
	gcc -o $@ $^ ${CXXFLAGS} ${LDFLAGS} ${LOADLIBS} ${EXAMPLELIBS} ${SEARCHDIRS} 

hsnTest/1_CreateCollieIO.exe: hsnTest/1_CreateCollieIO.o
	gcc -o $@ $^ ${CXXFLAGS} ${LDFLAGS} ${LOADLIBS} ${EXAMPLELIBS} ${SEARCHDIRS}

hsnTest/2_CalculateLimits.exe: hsnTest/2_CalculateLimits.o
	gcc -o $@ $^ ${CXXFLAGS} ${LDFLAGS} ${LOADLIBS} ${EXAMPLELIBS} ${SEARCHDIRS} 

tidy::
	@${RM} core ${ALLOBJS} ${COLLIEEXMP}/*.o ${COLLIEEXMP}/*.exe ${COLLIETOY}/*.o ${COLLIETOY}/*.exe ${COLLIETEST}/*.o ${COLLIETEST}/*.exe ${LIM_SRC}/wrap.o ${IO_SRC}/collieio_dict.cxx ${IO_SRC}/collieio_dict.h ${LIM_SRC}/collielimit_dict.cxx ${LIM_SRC}/collielimit_dict.h ${LIM_SRC}/collielimit_dict.o ${IO_SRC}/collieio_dict.o 
