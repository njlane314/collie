    ...
    for(int sigma_i = lsig; sigma_i <= hsig; ++sigma_i) {
      ...
      for (int i=0; i<len; i++) {
        if(v1[i]==mass || mass==-1){
          ...
          //
          f.cd();
          int bins = 500; double min = 1; double max = -1;
          TH1D* sigLLR = clcompute->getLLRdist_sb("LLR_SB",bins,min,max);
          TH1D* bkgLLR = clcompute->getLLRdist_b("LLR_B",bins,min,max);
          TH1D* LLRd = new TH1D("LLR_D","LLR_D",bins,clresults.llrobs*0.9,clresults.llrobs*1.1);
          
          LLRd->Fill(clresults.llrobs);
          cout << "LLR of data: " << clresults.llrobs << endl;
