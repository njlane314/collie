...
public:
  ...
  double func(double s, SigBkgdDist& dist, int level, double limit, int nSigma=0) {
    ...
    float retval = test.cls_med-limit;
    if(nSigma==-1) retval = test.cls_med_m1s-limit;
    if(nSigma==-2) retval = test.cls_med_m2s-limit;
    if(nSigma==1) retval = test.cls_med_p1s-limit;
    if(nSigma==2) retval = test.cls_med_p2s-limit;
    return retval;
  }
};
class ExclusionLimitobsFunc {
public:
  CLcompute* p_cl;
  double func(double s, SigBkgdDist& dist, int level, double limit, int nSigma=0) {
    static CLpoint test(100);
...
protected:
  double zriddr(double x1, double x2) {
	...
	if (m_verbose) printf("   ExclusionLimit: Found delta CL=%.5f%% at X=%f\n",fm*100.0,xm);	
	s=sqrt(fm*fm-fl*fh);
	if (s==0.0) return xm;
	...
	if (m_verbose) printf("   ExclusionLimit: Found 2nd delta CL=%.5f%% at X=%f\n",fnew*100.0,ans);
...
bool ExclusionLimit::calculate(const SigBkgdDist& dist, CLpoint& CLs) {
  ...
  Seeker1d<ExclusionLimitmedFunc> medSeeker;
  Seeker1d<ExclusionLimitobsFunc> obsSeeker;
...
/*
void ExclusionLimit::print(){
  printf("\n****************************************:\n");
  printf("\nExclusionLimit calculator results:\n");
  printf("==>CL level: %.4f, N sigma: %d\n",m_cllevel, m_sigma);
  printf("==>Accuracy: %.4f, Precision: %d\n",m_accuracy, m_precision);
  printf("==>Scaling factor Exp: %.3f, Obs: %.3f\n",m_expFact, m_obsFact);
  printf("****************************************:\n");
  return;
}
*/
