...
const char* CollieMasspoint::buildName(const char* channelname, int var1, int var2, int var3, int nvar) {
  char tool[1024];

  if (nvar>=3) sprintf(tool,"%s(%d,%d,%d)",channelname,var1,var2,var3);
  else if (nvar==2) sprintf(tool,"%s(%d,%d)",channelname,var1,var2);
  else if (nvar<=1) sprintf(tool,"%s(%d)",channelname,var1);
