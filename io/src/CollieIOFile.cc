...
//////////////////////////////
void CollieIOFile::createMassPoint(int par,  TH1D* data,  TH1D* sig,  double alphaS, vector<TH1D*>& bkg, vector<double>& alphaB){
  vector<TH1D*> sigV;
  sigV.push_back(sig);
  vector<double> sigA;
...
void CollieIOFile::createSigSystematic_Intl(int sigIdx, string syst, TH1D* pos, TH1D* neg, int parX, int parY, int parZ){
  ...
  for(int i=1; i<=pos->GetNbinsX();i++){
    ...
    if(std::isnan(pos->GetBinContent(i))!=0) nnan++;
    if(std::isnan(neg->GetBinContent(i))!=0) nnan++;
    if(std::isinf(pos->GetBinContent(i))!=0) ninf++;
    if(std::isinf(neg->GetBinContent(i))!=0) ninf++;
  }
  if(nnan!=0 || ninf!=0){
...
void CollieIOFile::createShapeSigSystematic2D(int sigIndex,string syst, TH2D* pos, TH2D* neg, int parX, int parY, int parZ,
  ...
  for(int i=1; i<=pos->GetNbinsX();i++){
    for(int j=1; j<=pos->GetNbinsY();j++){
      if(std::isnan(pos->GetBinContent(i,j))!=0) nnan++;
      if(std::isnan(neg->GetBinContent(i,j))!=0) nnan++;
      if(std::isinf(pos->GetBinContent(i,j))!=0) ninf++;
      if(std::isinf(neg->GetBinContent(i,j))!=0) ninf++;
    }  
  }
  if(nnan!=0 || ninf!=0){
